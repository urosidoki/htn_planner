...
if not exist "%HTN_PACKAGE_LIB%\HTNIntegration.lib" exit /b 1
if not exist "%HTN_EXAMPLE_BUILD%" mkdir "%HTN_EXAMPLE_BUILD%"
if errorlevel 1 exit /b 1

"%HTN_PACKAGE_BIN%\HTNTranslator.exe" "%~dp0example.domain" CreatePackageIntegrationConsumerHTN "%HTN_EXAMPLE_BUILD%"
if errorlevel 1 exit /b 1
...
if errorlevel 1 exit /b 1
cl.exe /nologo /std:c++20 /EHsc /W4 /WX %HTN_EXAMPLE_COMPILE% %HTN_EXAMPLE_DEFINES% /I"%HTN_PACKAGE_ROOT%\include\HTNFramework" /I"%HTN_PACKAGE_ROOT%\include\HTNIntegration" "%~dp0main.cpp" "%HTN_EXAMPLE_BUILD%\domain.obj" /Fo"%HTN_EXAMPLE_BUILD%\consumer.obj" /Fe"%HTN_EXAMPLE_BUILD%\IntegrationConsumer.exe" /link /MACHINE:X64 /LIBPATH:"%HTN_PACKAGE_LIB%" HTNIntegration.lib HTNFramework.lib
if errorlevel 1 exit /b 1
"%HTN_EXAMPLE_BUILD%\IntegrationConsumer.exe"
exit /b %ERRORLEVEL%
